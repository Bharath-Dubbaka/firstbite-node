...
router.get("/", async (req, res) => {
    try {
        ...
        if (status) { // e.g., ?status=delivered
            filter.orderStatus = status;
        }

        const orders = await Order.find(filter)
            .populate("userId", "name phoneNumber") // Get user's name and phone
            .sort({ createdAt: -1 });
    ...
    try {
        ...
        const updatedOrder = await Order.findByIdAndUpdate(
            req.params.id,
            {
                ...
                $push: { statusHistory: { status: status, note: note || `Status updated by admin`, timestamp: new Date() } }
            },
            { new: true }
        );
